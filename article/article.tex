        ...
        \begin{axis}[
            ...
            \addplot [
                ...
                error bars/.cd,
                    y dir=both,
                    y explicit,
            ] table [x=bits,y=mean,y error=error] {errorplot.dat};
            ...
            \addplot [
                ...
                error bars/.cd,
                    y dir=both,
                    y explicit,
            ] table [x=bits,y=commandMean,y error=commandError] {errorplot.dat};
            ...
            \addplot [
                ...
                error bars/.cd,
                    y dir=both,
                    y explicit,
            ] table [x=bits,y=precomMean,y error=precomError] {errorplot.dat};
            \addlegendentry{Precomputation}
        \end{axis} 
    \end{tikzpicture}
    \caption{Performance Evaluation of an increasing vector size and a constant 100 000 commands.}
...
%         xmin=0,
%         xticklabel={$2^{\pgfmathparse{8+\tick}\pgfmathprintnumber{\pgfmathresult}}$},
%         % x tick label as interval,
%         xlabel={vector length},
% 	]
    
%     % \addplot table[y=time, only if={entry of bits is 31}]{benchmark.dat}; 
%     % \addplot table[y=time, only if={entry of bits is 32}]{benchmark.dat}; 
%     \pgfplotsinvokeforeach{8,...,32}{ 
%         \addplot table[y=time,x=bits,only if={entry of bits is #1}]{benchmark.dat}; 
%         % \addlegendentry{Cond. $2^{#1}$};
%     } 
